...
import { LLMAsJudgeEvaluator, MockJudgeLLMProvider } from "./llm/eval/llm-as-judge.evaluator.js";
async function runCiEvaluation() {
    ...
    console.log("================================================================================");
    const REQUIRED_PASS_RATE = 100;
    if (report.passRate < REQUIRED_PASS_RATE) {
        console.error(`\nCI Regression Failure: Pass rate ${report.passRate}% is below required ${REQUIRED_PASS_RATE}%.`);
        process.exit(1);
    }
    else {
        console.log("\nCI Regression Passed: All evaluation criteria satisfied!");
        process.exit(0);
    }
}
