...
import {
  ...
  rewriteQuery,
  createLCELRetrievalPipeline,
} from "../rag/retrievers.js";
import type { LabeledEvaluationQuery, BenchmarkMetrics, RetrievalResult } from "../rag/types.js";
...
export function computeContextPrecision(retrievedIds: string[], relevantIds: string[], k: number): number {
  if (k === 0) return 0;
  const topK = retrievedIds.slice(0, k);
...
// ============================================================================
export function executeRawDay03Search(query: string, k = 3): RetrievalResult[] {
...
export async function executeLCELSearch(query: string): Promise<RetrievalResult[]> {
...
// ============================================================================
export async function runBenchmarkComparison(iterations = 20): Promise<{
  raw: PipelineSummary;
  lcel: PipelineSummary;
  delta: {
  ...
  const rawSummary: PipelineSummary = {
  ...
  const lcelSummary: PipelineSummary = {
