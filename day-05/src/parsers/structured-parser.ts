...
export function repairMalformedJson(corrupted: string): string {
...
export interface StructuredParserOptions {
  repairModel?: BaseChatModel;
  maxRetries?: number;
}

export class RetryableStructuredParser<T extends z.ZodTypeAny> {
  constructor(
    private readonly schema: T,
    private readonly options: StructuredParserOptions = {}
  ) {}
  ...
  toRunnable() {
    ...
    return RunnableLambda.from(async (rawText: string): Promise<z.infer<T>> => {
